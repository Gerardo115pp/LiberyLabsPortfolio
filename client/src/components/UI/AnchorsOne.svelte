<script>
    ...
    const control_point_difference = { dx: -231.3881, dy: 47.12554999999999}
    const central_square_position = {x: 289.0815, y: 50.015550000000005}
    const control_line = {
...
<svg  id="anchor-symbol" viewBox="0 0 {svg_width} {svg_height}" fill="none"preserveAspectRatio="none">
    <g id="anchor-element">
        ...
        <path data-name="main-curve-line" class="anchors-1-cls-2" d="M1.93344 0.195312Q{control_line.x1} {control_line.y1} {central_square_position.x} {central_square_position.y}T{end_point.x} {end_point.y}" stroke="#053160" stroke-miterlimit="10"/>
        <rect id="center-rect" x="{central_square_position.x}" y="{central_square_position.y}" width="{central_square_size}" height="{central_square_size}" transform="translate(-{central_square_size/2}, -{central_square_size/2})"/>
        <path data-name="guide-line" class="anchors-1-cls-2" d="M{visual_control_line.x1} {visual_control_line.y1}L{visual_control_line.x2} {visual_control_line.y2}"  stroke-miterlimit="10"/>
        <circle class="guide-controls" cx="{visual_control_line.x1}" cy="{visual_control_line.y1}" r={(central_square_size/2) * 1.4}/>
        <circle class="guide-controls" cx="{visual_control_line.x2}" cy="{visual_control_line.y2}" r={(central_square_size/2) * 1.4}/>
    </g>
</svg>    

<style>
    svg#anchor-symbol {
        max-width: calc(0.5816494140625 * var(--vspacing-8));
        height: 102px;
    }
